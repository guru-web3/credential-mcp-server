...
export const CreateCredentialTemplateArgsSchema = z
  .object({
    schemaId: z
      .string()
      .optional()
      .describe(
        'Schema ID to use. When provided, schemeType, schemeTitle and credentialName are fetched from the schema if not given.'
      ),
    credentialName: z
      .string()
      .optional()
      .describe('Name of the credential. Filled from schema title when schemaId is provided and this is omitted.'),
    schemeType: z
      .string()
      .optional()
      .describe('Schema type identifier. Filled from schema when schemaId is provided and this is omitted.'),
    schemeTitle: z
      .string()
      .optional()
      .describe('Schema title. Filled from schema when schemaId is provided and this is omitted.'),
    ...
    issueMax: z
      ...
      .optional()
      .default(null)
      .describe('Maximum number of credentials to issue (null for unlimited)'),
...
export async function createCredentialTemplate(args: z.infer<typeof CreateCredentialTemplateArgsSchema>) {
  ...
  if (!schemeType || !schemeTitle) {
    throw new Error(
      'Missing schemeType or schemeTitle. Provide them or pass schemaId so they can be loaded from the schema.'
    );
  }
  if (!credentialName) {
    throw new Error(
      'Missing credentialName. Provide it or pass schemaId so it can be defaulted from the schema title.'
    );
  }
  ...
  try {
    const response = await apiRequest<{
      ...
      id: number;
      status: string;
    }>('POST', '/issuer/credentialTemplateCreate', templateData, {
      'x-issuer-id': issuerId,
    });
...
export const createCredentialTemplateToolDefinition = {
  ...
  description:
    'Create an issuance program (credential template) for the issuer. Pass schemaId to fill schemeType, schemeTitle and credentialName from the schema; otherwise provide them or use session schemaId.',
  inputSchema: {
    type: 'object',
    properties: {
      schemaId: {
