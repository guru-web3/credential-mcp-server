...
export const SetPriceArgsSchema = z.object({
  paymentFeeSchemaId: z.string().optional().describe('Existing payment fee schema ID (bytes32 hex). Omit to create a new one.'),
...
export async function setPrice(args: SetPriceArgs): Promise<{ success: boolean; txHash: string; message: string; paymentFeeSchemaId?: string }> {
  if (!hasChainWallet()) {
